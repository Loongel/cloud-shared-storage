...
| Full profile prerequisites | `cs.backup=true`, `shared+multi+static`, and `shared+multi+sqlite/auto` require Kopia, Gluster, and LiteFS runtime config. | Environment/design gap | Preflight must detect prerequisites and either provision a test-local backend or mark the scenario `BLOCKED` before deploy. |
...
5. Shared multi capability:
   ...
   - provision Gluster/LiteFS test prerequisites through host service config or explicit preflight scripts;
...
- `shared-multi-only` passes in the intended topology or is documented as requiring multi-node provisioning.
