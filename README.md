...
- `CS_KOPIA_REPOSITORY`: legacy repository hint for `cs.backup=true`; use `CS_KOPIA_CONFIG_PATH` for a real connected Kopia repository config.
...
- `CS_KOPIA_SNAPSHOT_INTERVAL`: interval for `cs.backup=true` periodic Kopia snapshots, default `1h`; set `0` to keep only final snapshots on unmount/remove.
...
- `cs.backup=true|false`, default `false`.
...
```sh
./scripts/cs-storage-build-deb.sh --version 0.1.21
```
...
sh /tmp/cs-storage-install.sh \
  --deb-url https://github.com/Loongel/cloud-shared-storage/releases/download/v0.1.21/cs-storage_0.1.21_amd64.deb \
  --role all \
  --driver-name css \
...
DEB_BUILD_OK path=dist/cs-storage_0.1.5_amd64.deb sha256=2efe1783999cf59e9773ff52f23545940184550dc7a3171115c59c9a0345d7bd compression=control.tar.xz source=GitHub-Actions
