...
`node_secret` is not the node id. The node id defaults to `hostname` and can be
overridden with `--node-id`. `node_secret` is the shared cluster authentication
secret generated by the server/all installer; copy the server's
