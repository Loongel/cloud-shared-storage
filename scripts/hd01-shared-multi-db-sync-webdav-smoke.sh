...
SMOKE=${SMOKE:-/tmp/cs-storage-shared-multi-db-sync-webdav-smoke}
DRIVER=${DRIVER:-cs-storage-shared-db-sync-webdav-smoke}
VOLUME=${VOLUME:-cs-shared-db-sync-webdav-smoke-vol}
DAEMON_SOCKET=${DAEMON_SOCKET:-/run/cs-storage-shared-db-sync-webdav-smoke.sock}
PLUGIN_SOCKET=${PLUGIN_SOCKET:-/run/docker/plugins/cs-storage-shared-db-sync-webdav-smoke.sock}
ROOT_DIR=${ROOT_DIR:-/mnt/cs_storage/vols/.cs-shared-db-sync-webdav-smoke}
AUDIT_LOG=${AUDIT_LOG:-/var/log/cs-storage/shared-db-sync-webdav-smoke-audit.jsonl}
SERVER_PORT=${SERVER_PORT:-$((18142 + ($$ % 1000)))}
LITEFS_PORT=${LITEFS_PORT:-$((20342 + ($$ % 1000)))}
SECRET=${SECRET:-cs-shared-db-sync-webdav-smoke-secret}
NODE_ID=${NODE_ID:-$(hostname -s 2>/dev/null || hostname)}
REMOTE_ROOT=${REMOTE_ROOT:-cs-storage-shared-db-sync-webdav-smoke-$(date +%s)-$$}
...
APP_CONTAINER=${APP_CONTAINER:-cs-storage-shared-db-sync-webdav-smoke-app}
...
case "$DRIVER" in
  cs-storage-shared-db-sync-webdav-smoke*) ;;
  *) echo "REFUSE_DRIVER name=$DRIVER"; exit 1 ;;
esac
case "$DAEMON_SOCKET" in
  /run/cs-storage-shared-db-sync-webdav-smoke*.sock) ;;
  *) echo "REFUSE_DAEMON_SOCKET path=$DAEMON_SOCKET"; exit 1 ;;
esac
case "$PLUGIN_SOCKET" in
  /run/docker/plugins/cs-storage-shared-db-sync-webdav-smoke*.sock) ;;
  *) echo "REFUSE_PLUGIN_SOCKET path=$PLUGIN_SOCKET"; exit 1 ;;
esac
case "$ROOT_DIR" in
  /mnt/cs_storage/vols/.cs-shared-db-sync-webdav-smoke|/mnt/cs_storage/vols/.cs-shared-db-sync-webdav-smoke/*) ;;
  *) echo "REFUSE_ROOT_DIR path=$ROOT_DIR"; exit 1 ;;
esac
case "$AUDIT_LOG" in
  /var/log/cs-storage/shared-db-sync-webdav-smoke*) ;;
...
copy_tool_from_image() {
  ...
  if test "$copied" != 1; then
    echo "SHARED_DB_SYNC_PREREQ_MISSING tool=$tool image=$RUNTIME_IMAGE"
    exit 1
  fi
  chmod 700 "$out"
}
...
if ! command -v fusermount3 >/dev/null 2>&1 && ! command -v fusermount >/dev/null 2>&1; then
  echo "SHARED_DB_SYNC_PREREQ_MISSING tool=fusermount"
  exit 1
fi
if test ! -c /dev/fuse; then
  echo "SHARED_DB_SYNC_PREREQ_MISSING path=/dev/fuse"
  exit 1
fi
...
cleanup() {
  ...
  timeout 20s docker volume rm "$VOLUME" >/dev/null 2>&1 || true
...
CS_RCLONE_SYNC_SOURCE="$ROOT_DIR/{volume}/mount" \
...
docker rm -f "$APP_CONTAINER" >/dev/null 2>&1 || true
docker run -d --name "$APP_CONTAINER" --entrypoint sh -v "$VOLUME:/data" "$SQLITE_IMAGE" -c "sqlite3 /data/$DB_NAME \"PRAGMA journal_mode=WAL; PRAGMA busy_timeout=5000; CREATE TABLE IF NOT EXISTS events(node TEXT, n INTEGER, value TEXT); INSERT INTO events VALUES('$NODE_ID', 1, 'direct-db-sync'); PRAGMA wal_checkpoint(TRUNCATE); PRAGMA integrity_check;\" > /tmp/db.out && grep -q ok /tmp/db.out && sync && touch /data/.ready && sleep 300" >/dev/null
...
if ! docker exec "$APP_CONTAINER" test -f /data/.ready; then
  echo "SHARED_MULTI_DB_SYNC_APP_NOT_READY"
  docker logs "$APP_CONTAINER" 2>&1 || true
  exit 1
fi
remote_ok=0
rel="$REMOTE_ROOT/nodes/$NODE_ID/$DB_NAME"
for _ in $(seq 1 "$VERIFY_TIMEOUT"); do
  code=$(curl -sS -o "$SMOKE/remote-$DB_NAME" -w '%{http_code}' --netrc-file "$NETRC" "$BASE_URL/$rel" || true)
  if test "$code" = "200"; then
    if docker run --rm --entrypoint sqlite3 -v "$SMOKE:/verify" "$SQLITE_IMAGE" "/verify/remote-$DB_NAME" "PRAGMA integrity_check; SELECT COUNT(*) FROM events WHERE value='direct-db-sync';" > "$SMOKE/remote-db-check.out" 2>"$SMOKE/remote-db-check.err"; then
      if grep -q '^ok$' "$SMOKE/remote-db-check.out" && grep -q '^1$' "$SMOKE/remote-db-check.out"; then
        remote_ok=1
        break
      fi
    fi
...
if test "$remote_ok" != 1; then
  echo "SHARED_MULTI_DB_SYNC_WEBDAV_REMOTE_VERIFY_FAILED node=$NODE_ID remote_root=$REMOTE_ROOT rel=$rel"
  echo REMOTE_DB_CHECK
  sed -n '1,80p' "$SMOKE/remote-db-check.out" 2>/dev/null || true
  sed -n '1,80p' "$SMOKE/remote-db-check.err" 2>/dev/null || true
  echo SERVER_LOGS
...
docker rm -f "$APP_CONTAINER" >/dev/null 2>&1 || true
timeout 20s docker volume rm "$VOLUME"
test -s "$AUDIT_LOG"

trap - EXIT
cleanup

echo "SHARED_MULTI_DB_SYNC_WEBDAV_SMOKE_OK node=$NODE_ID driver=$DRIVER volume=$VOLUME remote_root=$REMOTE_ROOT db=$DB_NAME integrity=ok rows=1"
