...
SMOKE=${SMOKE:-/tmp/cs-storage-production-path-global-smoke}
NODES_MIN=${NODES_MIN:-}
DRIVER=${DRIVER:-cs-storage-prodpath-smoke}
VOLUME=${VOLUME:-cs-prodpath-smoke-vol}
SOCKET_NAME=${SOCKET_NAME:-cs-storage-prodpath-smoke.sock}
DAEMON_SOCKET=${DAEMON_SOCKET:-/run/$SOCKET_NAME}
ROOT_DIR=${ROOT_DIR:-/mnt/cs_storage/vols/.cs-prodpath-smoke}
AUDIT_LOG=${AUDIT_LOG:-/var/log/cs-storage/prodpath-smoke-audit.jsonl}
...
if test "$active_nodes" -lt "$NODES_MIN"; then
  echo "PRODUCTION_PATH_GLOBAL_NOT_ENOUGH_NODES ready=$active_nodes min=$NODES_MIN"
  exit 1
fi

case "$DRIVER" in
  cs-storage-prodpath-smoke*) ;;
  *) echo "REFUSE_DRIVER name=$DRIVER"; exit 1 ;;
esac
case "$ROOT_DIR" in
  /mnt/cs_storage/vols/.cs-prodpath-smoke|/mnt/cs_storage/vols/.cs-prodpath-smoke/*) ;;
  *) echo "REFUSE_ROOT_DIR path=$ROOT_DIR"; exit 1 ;;
esac
case "$DAEMON_SOCKET" in
  /run/cs-storage-prodpath-smoke*.sock) ;;
  *) echo "REFUSE_DAEMON_SOCKET path=$DAEMON_SOCKET"; exit 1 ;;
esac
case "$AUDIT_LOG" in
  /var/log/cs-storage/prodpath-smoke*) ;;
...
services:
  ...
  plugin:
    ...
    command:
      - |
        set -eu
        rm -f /run/docker/plugins/$DRIVER.sock
        cleanup() { rm -f /run/docker/plugins/$DRIVER.sock; }
        trap cleanup EXIT TERM INT
        CS_PLUGIN_SOCKET=/run/docker/plugins/$DRIVER.sock \\
        CS_DAEMON_SOCKET=$DAEMON_SOCKET \\
    ...
    command:
      ...
      - |
        ...
        while test "\$\$i" -lt 160; do
          if test -S /run/docker/plugins/$DRIVER.sock && test -S $DAEMON_SOCKET; then
            break
          fi
          i=\$\$((i + 1))
          sleep 0.25
        done
        test -S /run/docker/plugins/$DRIVER.sock
        ...
        test -s $AUDIT_LOG
        echo PRODUCTION_PATH_GLOBAL_NODE_OK driver=$DRIVER volume=$VOLUME root=$ROOT_DIR
        sleep 300
    volumes:
      - type: bind
...
for _ in $(seq 1 180); do
  ok=$(docker service logs --raw "$tester" 2>/dev/null | awk '$1 == "PRODUCTION_PATH_GLOBAL_NODE_OK" {n++} END {print n+0}')
...
cat "$logs"
ok=$(awk '$1 == "PRODUCTION_PATH_GLOBAL_NODE_OK" {n++} END {print n+0}' "$logs")
if test "$ok" -lt "$NODES_MIN"; then
  ...
  echo "PRODUCTION_PATH_GLOBAL_SMOKE_NOT_READY ok=$ok min=$NODES_MIN ready=$active_nodes image=$IMAGE tester=$TESTER_IMAGE driver=$DRIVER"
  exit 1
fi
...
echo "PRODUCTION_PATH_GLOBAL_SMOKE_OK nodes=$ok ready=$active_nodes image=$IMAGE tester=$TESTER_IMAGE driver=$DRIVER root=$ROOT_DIR"
